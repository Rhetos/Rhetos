...
namespace Rhetos.Host.Net.Logging
{
    public class RhetosNetCoreLogger : Rhetos.Logging.ILogger
    {
        ...
        public void Write(Rhetos.Logging.EventType eventType, Func<string> logMessage)
        {
            switch (eventType)
            {
                case Rhetos.Logging.EventType.Trace:
                    netCoreLogger.LogTrace(logMessage());
                    break;
                case Rhetos.Logging.EventType.Info:
                    netCoreLogger.LogInformation(logMessage());
                    break;
                case Rhetos.Logging.EventType.Warning:
                    netCoreLogger.LogWarning(logMessage());
                    break;
                case Rhetos.Logging.EventType.Error:
