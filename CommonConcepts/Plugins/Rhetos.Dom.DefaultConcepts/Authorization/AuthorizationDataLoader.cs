    ...
    public class AuthorizationDataLoader : IAuthorizationData
    {
        ...
        public PrincipalInfo GetPrincipal(string username)
        {
            ...
            if (principal.ID == default(Guid))
            {
                ...
                {
                    _logger.Info(() =>
                        $"Adding unregistered principal '{username}'. See {OptionsAttribute.GetConfigurationPath<RhetosAppOptions>()}:{nameof(RhetosAppOptions.AuthorizationAddUnregisteredPrincipals)} in web.config.");
