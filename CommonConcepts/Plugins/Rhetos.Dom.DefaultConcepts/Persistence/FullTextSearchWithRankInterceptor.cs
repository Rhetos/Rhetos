...
namespace Rhetos.Dom.DefaultConcepts.Persistence
{
    ...
    /// </summary>
    public class FullTextSearchWithRankInterceptor : IDbCommandInterceptor
    {
        ...
        {
            if (cmd.CommandText.Contains(DatabaseExtensionFunctions.InterceptFullTextSearchWithRankFunction))
            {
                var parseFtsQuery = new Regex(@"\(\[Rhetos\]\.\[" + DatabaseExtensionFunctions.InterceptFullTextSearchWithRankFunction + @"\]\((?<itemId>.+?), (?<pattern>.*?), (?<rankTop>.*?), (?<tableName>.*?), (?<searchColumns>.*?)\)\) = 1", RegexOptions.Singleline);
                ...
                foreach (var ftsQuery in ftsQueries.OrderByDescending(m => m.Index))
                {
                    ...
                    var validations = new[]
                    {
                        ...
                        new { Parameter = "rankTop", GeneratedSql = pattern, Format = rankFormat,
                            ...
                            InfoLinq = "a simple integer variable, not an expression," }, // Not mentioning string literals to simplify the message.
                    };
                    ...
                    string ftsSql = string.Format("{0} IN (SELECT [KEY] FROM CONTAINSTABLE({1}, {2}, {3}, {4}))",
                        itemId,
