    ...
    public class AutofacConfigurationTest
    {
        ...
        public void CorrectRegistrationsBuildTime()
        {
            ...
            using (var container = builder.Build())
            {
                ...
                TestAmbiguousRegistations(container,
                    expectedMultiplePlugins: ["Rhetos.Dsl.IDslModelIndex", "Rhetos.Extensibility.IGenerator", "AutoActivate"],
        ...
        public void CorrectRegistrationsRuntimeWithInitialization()
        {
            ...
            using (var rhetosHost = rhetosHostBuilder.Build())
            {
                ...
                TestAmbiguousRegistations(rhetosHost.GetRootContainer(),
                    expectedMultiplePlugins: ["Rhetos.Dsl.IDslModelIndex"],
        ...
        private void TestAmbiguousRegistations(IComponentContext container, IReadOnlyCollection<string> expectedMultiplePlugins = null, IDictionary<Type, string> expectedOverridenRegistrations = null)
        {
            expectedMultiplePlugins = expectedMultiplePlugins ?? [];
...
@"Activator = AdditionalSourceFilesListing (ReflectionActivator), Services = [Rhetos.AdditionalSourceFilesListing], Lifetime = Autofac.Core.Lifetime.CurrentScopeLifetime, Sharing = None, Ownership = OwnedByLifetimeScope, Pipeline = Autofac.Core.Pipeline.ResolvePipeline
...
Activator = ExternalTextReader (ReflectionActivator), Services = [Rhetos.Dsl.IExternalTextReader], Lifetime = Autofac.Core.Lifetime.RootScopeLifetime, Sharing = Shared, Ownership = OwnedByLifetimeScope, Pipeline = Autofac.Core.Pipeline.ResolvePipeline
...
Activator = InstalledPackages (ProvidedInstanceActivator), Services = [Rhetos.Deployment.InstalledPackages, AutoActivate], Lifetime = Autofac.Core.Lifetime.RootScopeLifetime, Sharing = Shared, Ownership = OwnedByLifetimeScope, Pipeline = Autofac.Core.Pipeline.ResolvePipeline
...
Activator = NullImplementation (ReflectionActivator), Services = [Rhetos.DatabaseGenerator.IConceptDatabaseGenerator], Lifetime = Autofac.Core.Lifetime.CurrentScopeLifetime, Sharing = None, Ownership = OwnedByLifetimeScope, Pipeline = Autofac.Core.Pipeline.ResolvePipeline
