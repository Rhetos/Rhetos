...
namespace CommonConcepts.Test
{
    ...
    public class RequiredPropertyTest
    {
        ...
        public void ValidData()
        {
            ...
            {
                scope.Resolve<ISqlExecuter>().ExecuteSql(new[] { "DELETE FROM TestRequired.Simple" });
        ...
        public void BoolProperty()
        {
            using (var scope = TestScope.Create())
            {
                ...
                Assert.AreEqual("a False, b True", TestUtility.DumpSorted(repository.TestRequired.Simple2.Query().ToList(), item => item.Name + " " + item.Tagged));
