...
namespace Rhetos.CommonConcepts.Test
{
    ...
    public class CommonAuthorizationProviderTest
    {
        ...
        private class MockRepositories : INamedPlugins<IRepository>
        {
            ...
            public IReadOnlyCollection<IRepository> GetPlugins(string name)
            {
...
Roles.", ReportCacheMisses(log3));
        ...
        public List<string> ClearCachePrincipalsRoles_GetAuthorization(double authorizationCacheExpirationSeconds, bool editSystemRole, bool commit = true)
        {
            ...
            cache.ClearCachePrincipals([principals[0]]);
            cache.ClearCacheRoles([roles[0].ID]);
            if (editSystemRole)
                cache.ClearCacheRoles([roles[2].ID]);
