...
"%~dp0"External\Omega\ReplaceRegEx.exe AssemblyInfo.cs "^(\[assembly: Assembly(File|Informational)?Version(Attribute)?\(\").+(\"\)\])$" "${1}%2${4}" || GOTO ERROREXIT
"%~dp0"External\Omega\ReplaceRegEx.exe AssemblyInfo.cs "^(\[assembly: AssemblyCompany\(\")(.*)(\"\)\])$" "${1}Omega software${3}" || GOTO ERROREXIT
"%~dp0"External\Omega\ReplaceRegEx.exe AssemblyInfo.cs "^(\[assembly: AssemblyCopyright\(\")(.*)(\b\d{4}\b)(\"\)\])$" "${1}Copyright (C) Omega software %YEAR%${4}" || GOTO ERROREXIT
"%~dp0"External\Omega\ReplaceRegEx.exe PackageInfo.xml "^(  \<Version\>).+(\<\/Version\>)$" "${1}%2${2}" || GOTO ERROREXIT
POPD
